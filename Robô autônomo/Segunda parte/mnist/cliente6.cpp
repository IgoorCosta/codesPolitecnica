...
#define RAZAO 0.8663
#define CONTRASTE 10.0 // 4.2
#define BRILHO -5.0 // -1.4
...
int main(int argc, char *argv[]) {
    ...
    VideoWriter vo(arquivo,CV_FOURCC('X','V','I','D'),16,Size(320+120,240));
    ...
    Mat_<COR> digito;
    Mat_<COR> digitoResize(120,120,merda);
    Mat_<COR> digitoBoxResize(120,120,merda);
    ...
    while (ch != 27)
    {
        ...
        converte(img,b);
        // calculo das escalas
        for (int i = 0; i < 10; i++) if (i > 0) escala[i] = escala[i-1] * RAZAO;
        ...
        for (int i = 0; i < 20; i++) for (int j = 0; j < 5; j++) pixel[i][j] = 0;
        ...
        Mat_<FLT> RNCC[10];
        bool escalasCC[10];
        for (int i = 0; i < 10; i++) escalasCC[i] = false;
